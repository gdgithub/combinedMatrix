...
def genDiagonalMatrix(o, rango=[0,1],manual=False):
    result = []
    for r in range(1,o+1):
        ...
        for c in range(1,o+1):
            if c > r:
                ...
                row_aux.append(rango[0] + (rango[1]-rango[0])*random.random() if manual==False else int(raw_input('D -> a{0}{1}: '.format(c,c))))
